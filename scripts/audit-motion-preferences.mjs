...
const checks = [
  ...
  ['background transition clears pending retry', source.includes('if (!appActive) {') && source.includes('clearHydrationRetry();') && source.includes('return;')],
  ['background transition returns to fail-safe motion state', source.includes('reduceMotionKnown = false;\n        publish({ reduceMotion: true });')],
  ...
  ['app resume disables motion until OS refresh', source.includes('clearHydrationRetry();\n      reduceMotionKnown = false;\n      publish({ reduceMotion: true });\n      hydrateReduceMotion(generation);')],
