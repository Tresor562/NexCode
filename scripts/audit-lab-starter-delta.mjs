...
assert.match(behavioralSource, /const activeKey = portableWorkspaceKey\(draft\.activeFile \?\? ''\)/, 'HTML entry resolution must honor an explicitly active HTML document');
assert.match(behavioralSource, /const rootIndex = entries\.find/, 'HTML entry resolution must keep the root index as the stable fallback for non-HTML editing');
assert.match(behavioralSource, /function hasCssRule\(draft: LabDraft\): boolean/, 'CSS checks must inspect stylesheet evidence associated with the resolved HTML entry');
assert.match(behavioralSource, /const linkedStyles = entry\.content\.match/, 'CSS checks must inspect explicit stylesheet links from the resolved HTML page');
assert.match(behavioralSource, /normalizeRelativeWorkspacePath\(href, entry\.filename\)/, 'linked stylesheet paths must be resolved relative to the active HTML page');
assert.match(behavioralSource, /normalizedPath\.toLowerCase\(\)\.endsWith\('\.css'\)/, 'CSS checks must accept portable linked stylesheet paths instead of hard-coding styles.css');
assert.match(behavioralSource, /normalizeRelativeWorkspacePath\('styles\.css', entry\.filename\)/, 'CSS fallback must stay colocated with the resolved HTML entry');
