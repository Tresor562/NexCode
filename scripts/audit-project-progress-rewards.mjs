...
expect(/projectId:\s*project\.id/, 'Canonical portfolio persistence must use the registered project id.');
expect(/title:\s*project\.title\.trim\(\)/, 'Canonical portfolio persistence must use registered product metadata.');
expect(/completedAt:\s*new Date\(completedAt\)\.toISOString\(\)/, 'Canonical portfolio persistence must normalize completion timestamps.');
...
if (!/canonicalProof/.test(existingProofBranch)) {
  throw new Error('Existing portfolio proof updates must persist canonical evidence rather than raw caller payloads.');
}
