...
expect(evidence, /for \(const \[filename, starterContent\] of Object\.entries\(starter\)\)[\s\S]*const earnedEvidence = changedCharacterEvidence\(starterContent, content\);[\s\S]*creditedConstructionFingerprints\.has\(evidenceFingerprint\)[\s\S]*score \+= earnedEvidence;/, 'Edited starter files must be scored once per normalized construction fingerprint.');
expect(evidence, /if \(filename in starter\) continue;[\s\S]*creditedConstructionFingerprints\.has\(evidenceFingerprint\)[\s\S]*score \+= addedFileConstructionEvidence/, 'Added files must reuse the starter dedupe boundary so copied credited work cannot multiply evidence.');
...
expect(evidence, /const earnedEvidence = changedCharacterEvidence\(starterContent, content\);/, 'Starter edits must pass through the bounded edit-span evidence calculation before deduplication.');
