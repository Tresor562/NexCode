...
expect(evidence, /const seenAddedFileEvidence = new Set<string>\(\);/, 'Added-file evidence must track normalized content already credited in the same workspace.');
expect(evidence, /const evidenceFingerprint = meaningfulEvidenceText\(content\);[\s\S]*seenAddedFileEvidence\.has\(evidenceFingerprint\)[\s\S]*seenAddedFileEvidence\.add\(evidenceFingerprint\)/, 'Duplicating the same added code under several filenames must not multiply project evidence.');
