...
expect(evidence, /for \(const \[filename, starterContent\] of Object\.entries\(starter\)\)[\s\S]*changedCharacterEvidence\(starterContent, content\) <= 0[\s\S]*creditedConstructionFingerprints\.add\(evidenceFingerprint\)/, 'Edited starter files must seed the evidence dedupe set before newly added files are scored.');
expect(evidence, /const evidenceFingerprint = meaningfulEvidenceText\(content\);[\s\S]*creditedConstructionFingerprints\.has\(evidenceFingerprint\)[\s\S]*creditedConstructionFingerprints\.add\(evidenceFingerprint\)[\s\S]*score \+= addedFileConstructionEvidence/, 'Duplicating credited starter or added code under another filename must not multiply project evidence.');
