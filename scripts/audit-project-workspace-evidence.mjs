...
expect(evidence, /const starterEvidence = new Set\([\s\S]*Object\.values\(starter\)[\s\S]*meaningfulEvidenceText\(content\)[\s\S]*filter\(Boolean\)/, 'Canonical starter content must be normalized into a comparison set before added files can earn evidence.');
expect(evidence, /if \(!meaningfulContent \|\| starterEvidence\.has\(meaningfulContent\)\) continue;/, 'Renamed or duplicated starter files must not earn construction evidence.');
