...
requirePattern(/\{ value: lesson\.title, weight: 60 \}[\s\S]*\{ value: lesson\.concept, weight: 45 \}[\s\S]*\{ value: canonicalSkillIds\(lesson\.skillIds\)\.join\(' '\), weight: 40 \}/, 'Lesson title, concept and canonical skill identities must outrank broad course metadata in learning search relevance.');
...
requirePattern(/const completed = completedLessonSet\(completedLessonIds\);[\s\S]*completed\.has\(lesson\.id\)/, 'Large learning paths must sanitize restored completion ids once and retain constant-time completion membership checks during search.');
...
requirePattern(/const completedSet = completedLessonSet\(completedLessonIds\);[\s\S]*completedSet\.has\(lesson\.id\)[\s\S]*completedSet\.has\(id\)/, 'Course navigation summaries must share the sanitized completion identity boundary and constant-time membership checks.');
requirePattern(/const MAX_SKILL_ID_CHARS = 160;/, 'Learning navigation skill identities must have an explicit size bound before they reach mastery lookups.');
requirePattern(/function canonicalSkillIds\([\s\S]*new Set<string>\(\)[\s\S]*typeof rawId !== 'string'[\s\S]*rawId\.trim\(\)[\s\S]*skillId\.length > MAX_SKILL_ID_CHARS[\s\S]*\/\[\\u0000-\\u001f\\u007f\]\/\.test\(skillId\)[\s\S]*skillIds\.add\(skillId\)/, 'Learning navigation must trim, bound, sanitize and deduplicate skill identities at one shared boundary.');
requirePattern(/function prerequisiteReadinessPenalty\([\s\S]*canonicalSkillIds\(lesson\.prerequisiteSkillIds\)[\s\S]*if \(!state\)[\s\S]*penalty \+= 40;[\s\S]*score < RECOMMENDATION_PREREQUISITE_GATE[\s\S]*Math\.min\(MAX_PREREQUISITE_PENALTY, penalty\)/, 'Default recommendations must canonicalize prerequisite ids, penalize missing or under-mastered prerequisites, and cap the penalty.');
requirePattern(/const skillStates = canonicalSkillIds\(lesson\.skillIds\)[\s\S]*\.map\(\(skillId\) => mastery\[skillId\]\)/, 'Recommendation mastery lookups must use canonical skill identities so duplicate or padded ids cannot distort due-review breadth or weakness ranking.');
requirePattern(/const due = canonicalSkillIds\(lesson\.skillIds\)\.some\(\(skillId\) => \{[\s\S]*return state \? reviewIsDue\(state\.nextReviewAt, now\) : false;/, 'Explicit due-review filtering must use the same canonical skill identity boundary as recommendation ranking.');
requirePattern(/function reviewIsDue\([\s\S]*const nowMs = now\.getTime\(\);[\s\S]*if \(!Number\.isFinite\(nowMs\)\) return false;[\s\S]*if \(!nextReviewAt\) return true;/, 'Due-review detection must fail closed when the local learning clock is invalid while keeping unscheduled existing mastery reviewable under a valid clock.');
requirePattern(/function reviewUrgencyBonus\([\s\S]*const nowMs = now\.getTime\(\);[\s\S]*if \(!Number\.isFinite\(nowMs\)\) return 0;[\s\S]*if \(!nextReviewAt\) return 12;/, 'Review urgency must contribute no bonus when the local learning clock is invalid.');
...
requirePattern(/const MAX_DUE_SKILL_BREADTH_BONUS = 12;/, 'Multi-skill review breadth must stay explicitly capped so broad lessons cannot dominate the recommendation feed.');
requirePattern(/function dueSkillBreadthBonus\([\s\S]*!Number\.isFinite\(dueSkillCount\) \|\| dueSkillCount <= 1[\s\S]*Math\.min\(MAX_DUE_SKILL_BREADTH_BONUS, \(Math\.floor\(dueSkillCount\) - 1\) \* 4\)/, 'Multi-skill review breadth must reject malformed counts, ignore single-skill reviews, grow gradually, and remain bounded.');
requirePattern(/const dueBreadth = dueSkillBreadthBonus\(dueStates\.length\);[\s\S]*if \(dueReview\) score \+= 70 \+ reviewUrgency \+ dueBreadth;/, 'Default recommendations must reward lessons that efficiently reinforce several currently due skills without changing explicit search relevance.');
...
requirePattern(/const isCompleted = completed\.has\(lesson\.id\);[\s\S]*if \(!isCompleted\) score \+= 45;[\s\S]*if \(dueReview\) score \+= 70 \+ reviewUrgency \+ dueBreadth;[\s\S]*if \(isCompleted && !dueReview\) score -= COMPLETED_NOT_DUE_PENALTY;/, 'Recommendation priority must derive completion once and use it consistently for new learning, multi-skill due review, and completed-not-due penalties.');
requirePattern(/if \(!isCompleted\) score -= prerequisiteReadinessPenalty\(lesson, mastery\);/, 'Prerequisite gating must sequence unseen material without suppressing an already-completed lesson that is due for spaced review.');
requirePattern(/const score = terms\.length[\s\S]*\? searchScore[\s\S]*: learningPriorityScore\(lesson, completed, mastery, now\);/, 'Pedagogical recommendation priority, including prerequisite penalties and review breadth, must apply only when the learner is not performing an explicit text search.');

console.log('Learning search audit OK: canonical intent, programming-language isolation, canonical skill identities, weighted search, scalable indexed traversal, fail-closed review clock semantics, bounded overdue urgency, bounded multi-skill review breadth, prerequisite-aware new-content sequencing, due-review preservation, mastery-safe recommendation ranking, and authored-order tie-breaking are protected.');
