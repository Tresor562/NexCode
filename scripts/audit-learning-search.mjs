...
requirePattern(/function learningPriorityScore\([\s\S]*if \(!completed\.has\(lesson\.id\)\) score \+= 45;[\s\S]*if \(dueReview\) score \+= 90;/, 'Default learning recommendations must strongly prioritize due review while still advancing incomplete lessons.');
...
console.log('Learning search audit OK: canonical intent, programming-language isolation, weighted search, scalable indexed traversal, due-review semantics, mastery-safe recommendation ranking, and authored-order tie-breaking are protected.');
