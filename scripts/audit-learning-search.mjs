...
requirePattern(/function reviewUrgencyBonus\([\s\S]*if \(nextReviewMs > nowMs\) return 0;[\s\S]*Math\.log2\(overdueDays \+ 1\)[\s\S]*MAX_REVIEW_URGENCY_BONUS/, 'Review urgency must grow sublinearly with overdue age, ignore future reviews, and remain capped.');
...
requirePattern(/function learningPriorityScore\([\s\S]*if \(!completed\.has\(lesson\.id\)\) score \+= 45;[\s\S]*if \(dueReview\) score \+= 70 \+ reviewUrgency;/, 'Default learning recommendations must strongly prioritize due review while scaling that priority by bounded overdue urgency.');
requirePattern(/const score = terms\.length[\s\S]*\? searchScore[\s\S]*: learningPriorityScore\(lesson, completed, mastery, now\);/, 'Pedagogical recommendation priority must apply only when the learner is not performing an explicit text search.');

console.log('Learning search audit OK: canonical intent, programming-language isolation, weighted search, scalable indexed traversal, due-review semantics, bounded overdue urgency, mastery-safe recommendation ranking, and authored-order tie-breaking are protected.');
