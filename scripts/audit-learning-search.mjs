...
requirePattern(/const isCompleted = completed\.has\(lesson\.id\);[\s\S]*if \(!isCompleted\) score \+= 45;[\s\S]*if \(dueReview\) score \+= 70 \+ reviewUrgency;[\s\S]*if \(isCompleted && !dueReview\) score -= COMPLETED_NOT_DUE_PENALTY;/, 'Recommendation priority must derive completion once and use it consistently for new learning, due review, and completed-not-due penalties.');
