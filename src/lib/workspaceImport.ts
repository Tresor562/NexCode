...
const TEXT_EXTENSIONS = new Set([
  'html','htm','css','scss','sass','less','js','jsx','mjs','cjs','ts','tsx','json','py','sql','md','txt','xml','yaml','yml','toml','ini','env','sh','bash','ps1','java','kt','kts','c','h','cpp','hpp','cs','go','rs','php','rb','dart','swift','vue','svelte','graphql','gql','csv','gitignore','dockerfile',
]);
...
function canReadAsText(file: File) {
  ...
  return size <= MAX_TEXT_BYTES && TEXT_EXTENSIONS.has(extension(file.name));
}
