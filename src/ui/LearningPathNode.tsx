...
export function LearningPathNode({
  ...
  const currentArrival = useRef(new Animated.Value(1)).current;
  ...
  const previousCurrentState = useRef<LearningPathNodeState>(state);
  ...
  useEffect(() => {
    const previous = previousCurrentState.current;
    previousCurrentState.current = state;
    const becameCurrent = previous !== 'current' && state === 'current';

    currentArrival.stopAnimation();
    if (!becameCurrent || reduceMotion || !appActive) {
      currentArrival.setValue(1);
      return;
    }
    ...
    return () => arrivalAnimation.stop();
  }, [appActive, currentArrival, reduceMotion, state]);
  ...
  useEffect(() => () => {
    ...
    completionTrail.stopAnimation();
    completionHalo.stopAnimation();
  }, [completionHalo, completionTrail, currentArrival, press]);
  ...
  return (
    <View style={[styles.row, { transform: [{ translateX: offset }] }]}>
      ...
      <View style={styles.nodeStack}>
        ...
        <Animated.View style={{ transform: [{ translateY }, { translateY: arrivalY }, { scale: nodeScale }, { scale: arrivalScale }] }}>
          <Pressable
...
const styles = StyleSheet.create({
  ...
  availableLabel: { color: theme.colors.primaryBright, fontSize: theme.type.label, fontWeight: '900', letterSpacing: .8 },
  lockedLabel: { color: theme.colors.textMuted, fontSize: theme.type.label, fontWeight: '900', letterSpacing: .8 },
  title: { color: theme.colors.text, fontSize: theme.type.title, fontWeight: '900', lineHeight: 24, marginTop: 5 },
  titleCurrent: { color: theme.colors.white, letterSpacing: -.1 },
  titleLocked: { color: theme.colors.textMuted },
  meta: { color: theme.colors.textMuted, fontSize: theme.type.caption, fontWeight: '700', marginTop: 5, lineHeight: 18 },
  metaCurrent: { color: theme.colors.textSecondary },
  lockedMeta: { color: theme.colors.textMuted, fontSize: theme.type.caption, fontWeight: '700', marginTop: 5, lineHeight: 18 },
  ...
  nextCueText: { color: theme.colors.primaryBright, fontSize: theme.type.label, fontWeight: '900', letterSpacing: .6 },
});