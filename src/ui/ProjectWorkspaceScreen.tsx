...
export function ProjectWorkspaceScreen({ project, stored, onSave, onBack }: {
  ...
  return (
    <View>
      ...
      {panel === 'files' ? <View style={styles.panel}><Text style={styles.panelTitle}>Projet</Text>{files.map((filename) => <Pressable key={filename} onPress={() => selectFile(filename)} style={[styles.file, draft.activeFile === filename && styles.fileActive]}><Text style={styles.fileIcon}>#</Text><Text style={styles.fileText}>{filename}</Text><Text style={styles.chevron}>›</Text></Pressable>)}</View> : null}
