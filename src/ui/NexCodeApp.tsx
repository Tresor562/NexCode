...
export default function NexCodeApp() {
  ...
  if (!state.onboardingComplete) {
    return <Onboarding name={draftName} goal={draftGoal} onName={setDraftName} onGoal={setDraftGoal} onFinish={() => setState((current) => ({ ...current, onboardingComplete: true, name: draftName.trim(), learningGoal: draftGoal }))} />;
  }

  if (labLesson && activeCourse) {
    return <DetailShell><LabWorkspaceScreen lesson={labLesson} stored={state.labDrafts[labLesson.id]} onSave={saveLabDraft} onComplete={completeLab} onBack={() => { setActiveLesson(labLesson); setLabLesson(null); }} /></DetailShell>;
  }
  if (activeLesson && activeCourse) {
    return <DetailShell><LessonFlowScreen course={activeCourse} lesson={activeLesson} state={state} onRecord={recordAttempt} onOpenLab={() => setLabLesson(activeLesson)} onBack={() => { setActiveLesson(null); setActiveCourse(null); setTab('Apprendre'); }} /></DetailShell>;
  }
  ...
  return (
    <Animated.View style={{ opacity: entrance, transform: [{ translateY: entrance.interpolate({ inputRange: [0, 1], outputRange: [8, 0] }) }] }}>
      ...
      <Pressable onPress={() => nextCourse && nextLesson ? onOpenLesson(nextCourse, nextLesson) : recentCourse && recentNext ? onOpenLesson(recentCourse, recentNext) : onOpenLearn()} style={({ pressed }) => [styles.nextCard, pressed && styles.quickPressed]}>
        ...
        <View style={styles.rowBetween}><Pill label="PROCHAINE ÉTAPE" tone="primary" /><Text style={styles.duration}>{session.estimatedMinutes || 10} min</Text></View>
        <Text style={styles.nextCourse}>{nextCourse?.title ?? recentCourse?.title ?? 'NexCode'}</Text>
        <Text style={styles.nextTitle}>{nextLesson?.title ?? recentNext?.title ?? 'Choisis ton premier parcours'}</Text>
        <View style={styles.progressBlock}><View style={styles.rowBetween}><Text style={styles.progressCaption}>Aujourd’hui</Text><Text style={styles.progressCaption}>{progress}%</Text></View><ProgressBar value={progress} /></View>
        <View style={styles.continueRow}><Text style={styles.continueText}>CONTINUER</Text><Text style={styles.continueArrow}>›</Text></View>
      </Pressable>
      ...
      <View style={styles.quickGrid}><QuickAction title="Code Lab" subtitle="Coder, tester, importer" onPress={onOpenLab} /><QuickAction title="Parcours" subtitle="Reprendre le chemin" onPress={onOpenLearn} /></View>
    </Animated.View>
  );
}
...
function Header({ streak, xp, nexCoins }: { streak: number; xp: number; nexCoins: number }) {
  return <View style={styles.header}><Text style={styles.wordmark}>NexCode</Text><View style={styles.headerStats}><Metric mark="S" value={streak} /><Metric mark="XP" value={xp} /><Metric mark="N" value={nexCoins} /></View></View>;
}

function Metric({ mark, value }: { mark: string; value: number }) {
  return <View style={styles.headerMetric}><View style={styles.metricMark}><Text style={styles.metricMarkText}>{mark}</Text></View><Text style={styles.metricValue}>{value}</Text></View>;
}
